...
use rust_openai_gpt_tools::service::spawn_openai_gpt_text_completion_socket_service;
...
async fn main() -> anyhow::Result<()> {
    ...
    if args.len() <= 1 {
        ...
        match args[1].as_str() {
            "start_service" => {
                spawn_openai_gpt_text_completion_socket_service("./tmp/rust_openai_gpt_tools_socket").await.unwrap();
                Ok(())
            },
            "test_service" => {
