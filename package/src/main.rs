...
use rust_openai_gpt_tools::my_completion_endpoint;
use rust_openai_gpt_tools::service::spawn_openai_gpt_summarization_socket_service;
...
async fn main() -> anyhow::Result<()> {
    ...
    if args.len() <= 1 {
        ...
        match args[1].as_str() {
            "start_service" => {
                spawn_openai_gpt_summarization_socket_service("./tmp/rust_openai_gpt_tools_socket").await.unwrap();
                Ok(())
            },
            "test_service" => {
                ...
                let result = client_send_openai_gpt_text_completion_request("./tmp/rust_openai_gpt_tools_socket", texts[0].to_owned(), PROMPTS[1].to_string(), 100)?;
                println!("{:?}",result);
                Ok(())
            }
            _ => {
